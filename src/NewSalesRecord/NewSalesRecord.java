...
public class NewSalesRecord extends JFrame {
...
double[] amountPerRows;
    ...
    public void populateCompany(){
        try{
            String sql = "SELECT company FROM company_name";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while(rs.next()){
               companyCombo.addItem(rs.getString("company"));
           }
        }catch(Exception e){
    ...
    public void populateCategory(){
        ...
        try{
            String sql = "SELECT product FROM product_category";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while(rs.next()){
    ...
    public void getPriceOfProductName(){
       ...
       if(des==0){
             try{
                    ...
                    JOptionPane.showMessageDialog(null, "Total Price = "+String.valueOf(sum));
    ...
    productCategoryCombo.removeAllItems();  // Clear existing items

    Set<String> categorySet = new HashSet<>(); // To avoid duplicate categories

    try {
        String sql = "SELECT product_name, product_category FROM products WHERE company_name=?";
        pst = conn.prepareStatement(sql);
        pst.setString(1, companyCombo.getSelectedItem().toString());  // Use prepared statements properly
        rs = pst.executeQuery();
        ...
        while (rs.next()) {
            ...
            // Log the values retrieved to debug
            System.out.println("Category: " + category + ", Product: " + productName);

            // Add product name to combo box
            productCombo.addItem(productName);

            // Ensure category is unique before adding
            if (!categorySet.contains(category)) {
                productCategoryCombo.addItem(category);
                categorySet.add(category);  // Mark this category as added
            }
        }
    } catch (Exception e) {
        JOptionPane.showMessageDialog(null, e);
    ...
    try {
        String selectedProduct = productCombo.getSelectedItem().toString();
        
        String sql = "SELECT product_category FROM products WHERE product_name=?";
        pst = conn.prepareStatement(sql);
        ...
        if (rs.next()) {
            String category = rs.getString("product_category");
            productCategoryCombo.setSelectedItem(category);  // Set the product category based on selected product
            
            // Log to check if correct category is set
